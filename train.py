...
from models.dagcn_bert import DAGCNBert
...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        if 'bert' in opt.model:
            ...
            trainset = ABSABertData(opt.dataset_file['train'], tokenizer, opt=opt, dep_vocab=dep_vocab)
            validset = ABSABertData(opt.dataset_file['valid'], tokenizer, opt=opt, dep_vocab=dep_vocab)
            testset = ABSABertData(opt.dataset_file['test'], tokenizer, opt=opt, dep_vocab=dep_vocab)
        else:    
            ...
            logger.info("token_vocab: {}, post_vocab: {}, dep_post_vocab: {}, pos_vocab: {}, dep_vocab: {}, pol_vocab: {}".format(len(token_vocab), len(post_vocab), len(dep_post_vocab), len(pos_vocab), len(dep_vocab), len(pol_vocab)))
    ...
    def _train(self, criterion, optimizer, max_valid_acc_overall=0):
        max_valid_acc = 0
        max_valid_f1 = 0
        global_step = 0
        model_path = ''
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_dataloader):
                ...
                if global_step % self.opt.log_step == 0:
                    ...
                    logger.info('loss: {:.4f}, acc: {:.4f}, valid_acc: {:.4f}, valid_f1: {:.4f}'.format(loss.item(), train_acc, valid_acc, f1))
                    if valid_acc > max_valid_acc:
                        ...
                        if valid_acc > max_valid_acc_overall:
                            ...
                            model_path = 'results/{}/{}_{}_acc_{:.4f}_f1_{:.4f}'.format(self.opt.model, self.opt.model, self.opt.dataset, valid_acc, f1)
    ...
    def run(self):
        ...
        max_valid_acc, max_valid_f1, model_path, checkpoint = self._train(criterion, optimizer)
    ...
    def testDetail(self):
        ...
        with torch.no_grad():
            for batch, sample_batched in enumerate(self.test_dataloader):
                ...
                targets = sample_batched['polarity'].to(self.opt.device)
                outputs, penal = self.model(inputs)
                for i, text, aspect, target, predicted in zip(((torch.argmax(outputs, -1) == targets)), inputs[0], inputs[1], targets, torch.argmax(outputs, -1)):
                    if not i:
                        tok_list = text.numpy()
